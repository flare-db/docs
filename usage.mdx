...
FlareDB runs Apache Beam pipelines as jobs. Build your pipeline with the Beam SDK as usual, then set FlareRunner as the runner to submit it to a running FlareDB instance.
## Prerequisites
...
Set `FlareRunner` as the runner and configure the FlareDB instance and application JAR:
...
See the full pipeline in the [WordCount example](https://github.com/flare-db/flare-db/tree/main/example/wordcount/src/main/java/com/flaredb/example).
